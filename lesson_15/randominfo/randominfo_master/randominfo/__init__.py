...
def get_address():
	...
	countries = ['Ukraine', 'India', 'USA', 'Germany', 'France', 'Brazil', 'Japan']
	try:
		with open(full_path('data.csv'), 'r', encoding='utf-8') as f:
			reader = list(csv.reader(f))
	except FileNotFoundError:
		return {param: 'N/A' for param in addrParam}
	full_addr = {}
	for param in addrParam:
		if param == 'country':
			full_addr['country'] = choice(countries)
		else:
			idx = column_indices.get(param)
			values = [row[idx].strip() for row in reader if len(row) > idx and row[idx].strip()]
			full_addr[param] = choice(values) if values else 'N/A'
	return full_addr
