...
car_data_sorted = dict(sorted(car_data.items(), key = lambda item: item[1][4]))

result = 0
for key, value in car_data_sorted.items():
  if value[1] >= search_criteria[0] and value[2] >= search_criteria[1] and value[4] <= search_criteria[2] and result < 5:
    result = result + 1
    print(key, value)
